...
function GameEventBanner({ event }) {
  useEffect(() => {
    ...
    const sound = soundByEvent[event.kind]
...
function GameSfxController({ state }) {
  ...
  useEffect(() => {
    ...
    if (!previous) {
      if (state.phase === 'dealing') playSfx('shuffle', 0.55)
      previousRef.current = state
      return
    }

    if (state.phase === 'dealing' && previous.phase !== 'dealing') {
      playSfx('shuffle', 0.55)
    }

    if ((state.discard?.length || 0) > (previous.discard?.length || 0)) {
      playSfx('cardPlace', 0.62)
    }

    if (state.hasDrawnThisTurn && !previous.hasDrawnThisTurn) {
    ...
    if (state.phase === 'round-over' && previous.phase !== 'round-over') {
      playSfx('winner', 0.78)
    }

    const currentPlayed = state.recentPlayCards?.[state.recentPlayCards.length - 1]
    const previousPlayed = previous.recentPlayCards?.[previous.recentPlayCards.length - 1]
    if (
      currentPlayed?.rank === 'A' &&
      currentPlayed?.id !== previousPlayed?.id &&
